...
(def car-type-prices {:bmw 60000 :ferrari 100000 :fiat 20000})

(def coupon-discount-pct 20)
(def valid-coupon-code "my-valid-coupon")

(defn analyze-budget
  "Returns an analysis of the supplied budget and coupon code"
  [budget code]
  (let
    [coupon-valid (= code valid-coupon-code)
     factor (if coupon-valid (- 1 (/ coupon-discount-pct 100)) 1)
     adjusted (map (fn [[type price]] [type (* price factor)]) car-type-prices)
     affordable (filter (fn [[type price]] (<= price budget)) adjusted)]
